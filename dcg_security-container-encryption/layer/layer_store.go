...
func (ls *layerStore) CreateRWLayer(name string, parent ChainID, opts *CreateRWLayerOpts) (RWLayer, error) {
	logrus.Debugf("layer_store: CreateRWLayer => name: %s, parent: %s, opts: %s ", name, parent, opts)
...
func (ls *layerStore) getTarStream(rl *roLayer) (io.ReadCloser, error) {
	if !ls.useTarSplit {
		...
		logrus.Debugf("layer_store: getTarStream (no useTarSplit)")
		return ls.driver.Diff(rl.cacheID, parentCacheID)
	}

	logrus.Debugf("layer_store: getTarStream (useTarSplit)")
...
func (ls *layerStore) assembleTarTo(graphID string, metadata io.ReadCloser, size *int64, w io.Writer) error {
	logrus.Debugf("layer_store: assembleTarTo => graphID: %s", graphID)

	diffDriver, ok := ls.driver.(graphdriver.DiffGetterDriver)
	if !ok {
		logrus.Debugf("layer_store: assembleTarTo (no DiffGetterDriver) ")
		diffDriver = &naiveDiffPathDriver{ls.driver}
	}

	logrus.Debugf("layer_store: assembleTarTo (DiffGetterDriver implemented) ")
