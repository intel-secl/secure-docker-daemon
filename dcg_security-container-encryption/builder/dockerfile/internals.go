...
func (b *Builder) runContextCommand(args []string, allowRemote bool, allowLocalDecompression bool, cmdName string, imageSource *imageMount) error {
	logrus.Debugf("runContextCommand: storage opts: %s", b.options.StorageOpt)
	...
	secStorageOpts, ok := b.getSecureStorageOpts()
	if !ok {
	...
	container, err := b.docker.ContainerCreate(types.ContainerCreateConfig{
		...
		HostConfig: &container.HostConfig{LogConfig: defaultLogConfig, StorageOpt: secStorageOpts},
	})
	if err != nil {
		return err
	}
...
func (b *Builder) create() (string, error) {
	logrus.Debugf("Builder: create => args: %v options: %s", *b.buildArgs, b.options.StorageOpt)
	...
	storageOpts, ok := b.getSecureStorageOpts()
	if !ok {
		return "", errors.New("Invalid storage options")
	}
...
func (b *Builder) getSecureStorageOpts() (map[string]string, bool) {
	// add extra args to storage opts
