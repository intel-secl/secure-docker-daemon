...
func (cli *DaemonCli) start(opts daemonOptions) (err error) {
	...
	_ = logrus_stack.StandardHook() // to make sure it doesn't complain about import ...
	if logFormatter, logFormatterOverrule := os.LookupEnv("DOCKER_LOG_FORMATTER"); !logFormatterOverrule {
		// original/default log settings
		logrus.SetFormatter(&logrus.TextFormatter{
			TimestampFormat: jsonlog.RFC3339NanoFixed,
			DisableColors:   cli.Config.RawLogs,
		})
	} else {
		logrus.Warnf("logrus configuration DOCKER_LOG_FORMATTER='%s' will make 'make test' of docker fail!", logFormatter)
		switch strings.ToLower(logFormatter) {
		case "jsonstack":
			logrus.AddHook(logrus_stack.NewHook(logrus.AllLevels, logrus.AllLevels))
			logrus.SetFormatter(&logrus.JSONFormatter{
				TimestampFormat: jsonlog.RFC3339NanoFixed,
			})
		case "jsonstackonerror":
		...
		default:
			return fmt.Errorf("Unknown DOCKER_LOG_FORMATTER: %v", logFormatter)
		}
	}
