...
func (daemon *Daemon) SquashImage(id, parent string) (string, error) {
	...
	for i, hi := range newImage.History {
		if i >= len(parentImg.History) {
			hi.EmptyLayer = true
			hi.EncSecurityOpts = "" // reduce clutter in history
		}
	...
	if len(parent) > 0 {
        ...
        encSecOpts := ""
	if meta, err := newL.Metadata(); err == nil {
        ...
        logrus.Debugf("daemon: Commit => squashed layer %s got image history (encoded) security opts : %v", newL.DiffID(), encSecOpts)

	newImage.History = append(newImage.History, image.History{
		Created: now,
		Comment: historyComment,
		EncSecurityOpts: encSecOpts,
	})
