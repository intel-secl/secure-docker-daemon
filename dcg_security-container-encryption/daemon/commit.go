...
func (daemon *Daemon) Commit(name string, c *backend.ContainerCommitConfig) (string, error) {
	logrus.Debugf("daemon: Commit => Commit container %s", name)
	...
	defer layer.ReleaseAndLog(daemon.layerStore, l)
        ...
        // save driver-specific storage/security meta-data in image history
        encSecOpts := ""
	if meta, err := container.RWLayer.Metadata(); err == nil {
		for mKey, val := range meta {
			// TODO: change to some constant and/or case-normalize on external input (program should use constant and hence fix case)
			if (strings.EqualFold(mKey, "security-meta-data")) {
				encSecOpts = val

				// TODO: replace this kludgy way with something cleaner ...
				if b,_ := regexp.MatchString("(?i)\"RequiresConfidentiality\"\\s*:\\s*true\\b", encSecOpts); b {
					// if confidentiality was required, also sanitize comment field
					logrus.Debugf("Sanitize history for confidential layer by scrubbing Author: %s, container-config-cmd: %v, Comment: '%s', ", c.Author, container.Config.Cmd, c.Comment)
					c.Author = "(private image layer: no author)"
					// see api/types/container/config.go for fields of Config
					container.Config.Cmd = []string{ "(private image layer: no command" }
					c.Comment = "(private image layer: no comments)"
				}
				break
                        }
                }
        }
        logrus.Debugf("daemon: Commit => image history (encoded) security opts: %v", encSecOpts)
	...
	h := image.History{
		...
		Comment:    c.Comment,
		EmptyLayer: true,
		EncSecurityOpts: "", // We do add them only if it is not an empty layer, see below
	}

	if diffID := l.DiffID(); layer.DigestSHA256EmptyTar != diffID {
		logrus.Debugf("daemon: Commit => layer with diffID %s is marked as empty", diffID)
		h.EmptyLayer = false
		h.EncSecurityOpts = encSecOpts
