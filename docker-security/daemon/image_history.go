...
func (daemon *Daemon) ImageHistory(name string) ([]*image.HistoryResponseItem, error) {
	...
	for _, h := range img.History {
		...
		encSecOpts := ""
		if h.SecurityOpts != nil {
			bytes, err := json.Marshal(h.SecurityOpts)
			if err != nil {
				return nil, fmt.Errorf("can't encode security options")
			}
			encSecOpts = string(bytes)
		}
