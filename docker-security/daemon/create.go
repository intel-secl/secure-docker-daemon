...
func (daemon *Daemon) create(params types.ContainerCreateConfig, managed bool) (retC *container.Container, retErr error) {
	...
	logrus.Debugf("daemon: create => storageopt: %s", container.HostConfig.StorageOpt)
...
func (daemon *Daemon) setRWLayer(container *container.Container) error {
	logrus.Debugf("daemon create: setRWLayer => storage opts: %v", container.HostConfig.StorageOpt)
	
	var layerID layer.ChainID
	storageOpts := make(map[string]string)

	// set secure storage options for container
	//  moving here to support "scratch" image building
	var secOpts []map[string]string
	
	// first element in the array will always store options for RW layer
	//   first element will be filled while building the image, it will be
	//	 empty when container is launcher after build
	rwlopt := make(map[string]string)
	for k, v := range container.HostConfig.StorageOpt {
		rwlopt[k] = v
		delete(container.HostConfig.StorageOpt, k)
	}
	...
	if container.ImageID != "" {
		...
		for _, h := range img.History {
			...
			for k, v := range h.SecurityOpts {
				m[k] = v
			}

			if !h.EmptyLayer {
				// History encodes information from bottom to top layer in sequential order
				//		while layer mouting is from top to bottom, so reversing meta-data
				//		information order from history
				secOpts = append([]map[string]string{m}, secOpts...)
				//secOpts = append(secOpts, m)
			}
		}

		layerID = img.RootFS.ChainID()
		logrus.Debugf("setRWLayer: imageID: %v, layerID: %v, diffIDs: %v, security opts: %s", container.ImageID, layerID, img.RootFS.DiffIDs, storageOpts["SecurityOpts"])
		//storageOpts["diffids"] = fmt.Sprintf("%v", img.RootFS.DiffIDs)
	}

	secOpts = append([]map[string]string{rwlopt}, secOpts...)

	// convert security options into json
	if (len(secOpts) > 0) && (daemon.layerStore.DriverName() == "secureoverlay2"){
		j, err := json.Marshal(secOpts)
		if err == nil {
			storageOpts["SecurityOpts"] = string(j)
		}
	}
