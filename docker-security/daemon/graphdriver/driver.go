...
func New(name string, pg plugingetter.PluginGetter, config Options) (Driver, error) {
	// add secureoverlay2 specific options here
	if name == "secureoverlay2" {
		config.DriverOptions = append(config.DriverOptions, fmt.Sprintf("remoteroot=%s", config.RemoteRoot))
	}
