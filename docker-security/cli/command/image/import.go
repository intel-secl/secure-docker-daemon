...
type importOptions struct {
	...
	message   	string
	storageopt  dockeropts.ListOpts
}
...
func NewImportCommand(dockerCli *command.DockerCli) *cobra.Command {
	...
	opts.storageopt = dockeropts.NewListOpts(nil)
	flags.VarP(&opts.storageopt, "storageopt", "s", "Options for docker image storage backend")

	return cmd
}

func runImport(dockerCli *command.DockerCli, opts importOptions) error {
	fmt.Println("runImport: opts: %s", opts.storageopt.GetAll())
	...
	options := types.ImageImportOptions{
		...
		StorageOpt: opts.storageopt.GetAll(),
	}
