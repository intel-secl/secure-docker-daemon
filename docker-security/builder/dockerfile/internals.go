...
func (b *Builder) runContextCommand(args []string, allowRemote bool, allowLocalDecompression bool, cmdName string, imageSource *imageMount) error {
	logrus.Debugf("runContextCommand: storate opts: %s", b.options.StorageOpt)
	...
	container, err := b.docker.ContainerCreate(types.ContainerCreateConfig{
		...
		HostConfig: &container.HostConfig{LogConfig: defaultLogConfig, StorageOpt: storageOpts},
	})
	if err != nil {
		return err
	}
	...
	if storageOpts["isConfidential"] == "true" {
		comment = fmt.Sprintf("(private image layer: no comments)")
	}
