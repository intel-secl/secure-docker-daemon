...
import (
	...
	_"github.com/docker/docker/pkg/parsers"
)
...
func NewStoreFromOptions(options StoreOptions) (Store, error) {
	driver, err := graphdriver.New(options.GraphDriver, options.PluginGetter, graphdriver.Options{
		...
		RemoteRoot:          options.RemoteStorePath,
	...
	logrus.Debugf("layer store: Using graph driver %s", driver)
	logrus.Debugf("layer store: StoreOpts: %s, DriverOpts: %s", options, options.GraphDriverOptions)
	
	remoteLayerDBLoc := ""
	if options.RemoteStorePath != "" {
		remoteLayerDBLoc = fmt.Sprintf("%s/image/%s/layerdb", options.RemoteStorePath, driver)
	}
...
func NewStoreFromGraphDriver(store MetadataStore, driver graphdriver.Driver) (Store, error) {
	...
	logrus.Debugf("layer stote: old layers loaded => ids: %v, mounts: %v", ids, mounts)
...
func (ls *layerStore) processStorageOpts(opts string, id string, parent ChainID) string {
	logrus.Debugf("layer store: processStorageOpts => id: %s, opts: %s", id, opts)
	
	if opts == "" {
		return opts
	}
	
	secOpts := make(map[string]map[string]string)
	...
	if length > 0 {
		s := o[0]
		// device status for new layer is always false in the begining
		if len(s) != 0 {
		...
		// get diffid and cache id of all lower layers
		idmap := ls.mapDiffIDCacheID(parent)
		logrus.Debugf("processStorageOpts: idmap: %v", idmap)

		// meta-data fetched from image history, DeviceStatus is always true for these layers
		index := 1
		for _, e := range idmap {
			s := o[index]
			if len(s) != 0 {
	...
	bytes, err := json.Marshal(secOpts)
	logrus.Debugf("processStorageOpts: opts: %s", string(bytes))
...
func (ls *layerStore) mapDiffIDCacheID(id ChainID) []string {
	m := []string{}
	...
	lid := id
	for string(lid) != "" {
		diffid, err := ls.store.GetDiffID(lid)
		if err != nil {
			return m
		}

		cacheid, err := ls.store.GetCacheID(lid)
		if err != nil {
			return m
		}

		sd = strings.Split(string(diffid), ":")[1]
		sc = string(cacheid)

		m = append(m, fmt.Sprintf("%s:%s",sd,sc))
		
		logrus.Debugf("secureoverlay2: mapDiffIDCacheID => added data for id: %s, m: %v", lid, m)
		p, err := ls.store.GetParent(lid)
		if err == nil {
			lid = p
		} else {
			lid = ""
...
func (ls *layerStore) CreateRWLayer(name string, parent ChainID, opts *CreateRWLayerOpts) (RWLayer, error) {
	logrus.Debugf("layer store: CreateRWLayer => name: %s, parent: %s, opts: %s ", name, parent, opts)
	...
	tmp := ""
	if len(storageOpt) > 0 {
		tmp = storageOpt["SecurityOpts"]
	}
	secopts := ls.processStorageOpts(tmp, m.mountID, parent)
	if secopts != "" {
		storageOpt["SecurityOpts"] = secopts
	}
...
func (ls *layerStore) convertDiffIDs2CacheIDs(l Layer) {
	lcid := l.ChainID()
	if string(lcid) != "" {
		cacheid, err := ls.store.GetCacheID(l.ChainID())
		logrus.Debugf("CreateRWLayer: layer: id: %s, cache: %s, err: %s", l.DiffID(), cacheid, err.Error())
	}
}
...
func (ls *layerStore) getTarStream(rl *roLayer) (io.ReadCloser, error) {
	if !ls.useTarSplit {
		...
		logrus.Debugf("aufs: getTarStream (no useTarSplit)")
		return ls.driver.Diff(rl.cacheID, parentCacheID)
	}

	logrus.Debugf("aufs: getTarStream (useTarSplit)")
...
func (ls *layerStore) assembleTarTo(graphID string, metadata io.ReadCloser, size *int64, w io.Writer) error {
	logrus.Debugf("aufs: assembleTarTo => graphID: %s", graphID)

	diffDriver, ok := ls.driver.(graphdriver.DiffGetterDriver)
	if !ok {
		logrus.Debugf("aufs: assembleTarTo (no DiffGetterDriver) ")
		diffDriver = &naiveDiffPathDriver{ls.driver}
	}

	logrus.Debugf("aufs: assembleTarTo (DiffGetterDriver implemented) ")
